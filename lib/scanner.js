...
async function checkHttpHeaders(url) {
  ...
  await assertNotPrivate(hostname);
  const result = {
    url,
    statusCode: null,
    server: null,
...
async function checkSsl(url) {
  ...
  try {
    ...
    if (parsed.protocol === 'https:') {
      ...
      // Verify actual TLS by making an HTTPS request and inspecting the socket
      try {
        const tlsResp = await axios.get(url, {
          timeout: DEFAULT_TIMEOUT,
          validateStatus: () => true,
          httpsAgent: new (require('https').Agent)({ rejectUnauthorized: true }),
        });
        result.grade = 'A';
        result.statusCode = tlsResp.status;

        // Check HSTS
        if (tlsResp.headers['strict-transport-security']) {
          result.hsts = true;
          const hstsVal = tlsResp.headers['strict-transport-security'];
          if (!hstsVal.includes('includeSubDomains')) result.issues.push('HSTS missing includeSubDomains');
          if (!hstsVal.includes('preload')) result.issues.push('HSTS missing preload directive');
        } else {
          result.hsts = false;
          result.issues.push('HSTS header not present');
          result.grade = 'B';
        }
      } catch (tlsErr) {
        if (tlsErr.code === 'CERT_HAS_EXPIRED') {
          result.issues.push('SSL certificate has expired');
          result.grade = 'F';
        } else if (tlsErr.code === 'DEPTH_ZERO_SELF_SIGNED_CERT' || tlsErr.code === 'SELF_SIGNED_CERT_IN_CHAIN') {
          result.issues.push('Self-signed certificate detected');
          result.grade = 'F';
        } else if (tlsErr.code === 'ERR_TLS_CERT_ALTNAME_INVALID') {
          result.issues.push('Certificate hostname mismatch');
          result.grade = 'F';
        } else {
          result.grade = 'B';
        }
      ...
      try {
        ...
        if (httpResp.status >= 300 && httpResp.status < 400) {
          ...
          result.httpRedirect = loc.startsWith('https://');
          if (!result.httpRedirect) result.issues.push('HTTP redirects but not to HTTPS');
        } else {
        ...
        result.httpRedirect = true; // likely redirects before connection closes
      }
...
async function checkCors(url) {
  ...
  try {
    ...
    if (acao === '*' && acac === 'true') {
      ...
      result.details.push('CRITICAL: Wildcard CORS with credentials allowed');
      result.allowOrigin = '*';
    } else if (acao === '*') {
      result.misconfigured = false;
      result.details.push('Wildcard CORS - public API (no credentials)');
      result.allowOrigin = '*';
