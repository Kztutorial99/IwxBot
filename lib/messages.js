...
// Escape characters that break Telegram Markdown v1 when inside non-code spans
function escMd(str) {
  if (typeof str !== 'string') str = String(str);
  return str.replace(/[_*[\]()~`>#+=|{}.!-]/g, '\\// Message templates for VulnScan Telegram Bot
...
function moduleInfoMessage(module) {
  const info = {
    ...
    ssl: {
      ...
      desc: 'Verifies HTTPS usage and checks if HTTP traffic is properly redirected to HTTPS.',
      payloads: ['None - passive check only'],
      risk: 'HIGH',
    },
    cors: {
